...
for ($d = 1; $d <= 2; $d++) { // 1 
	...
	for ($i = 1; $i <= 7; $i++) { //divre
		...
		if ($d === 1) {
            $url ="https://dashboard.telkom.co.id/cbd/summaryeksekutif/detilnetaddlokasi?header=PSBLN&level=REG&kolom=DIVRE%20".$i."&startdate=".$kemarin->format('d/m/Y')."&enddate=".$kemarin->format('d/m/Y')."&TEMATIK=3P&JENIS=ALL&ACTIVATION=ALL&DIVRE=ALL&WITEL=ALL&CHANEL=ALL&jenisalpro=ALL&segment=ALL&CCAT=ALL&dl=true";
            $periode = $kemarin->format('Y') . $kemarin->format('m');
		} else {
            $url ="https://dashboard.telkom.co.id/cbd/summaryeksekutif/detilnetaddlokasi?header=PSBLN&level=REG&kolom=DIVRE%20".$i."&startdate=".$hari_ini->format('d/m/Y')."&enddate=".$hari_ini->format('d/m/Y')."&TEMATIK=3P&JENIS=ALL&ACTIVATION=ALL&DIVRE=ALL&WITEL=ALL&CHANEL=ALL&jenisalpro=ALL&segment=ALL&CCAT=ALL&dl=true";
